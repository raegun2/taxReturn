...
function ContactForm() {
    ...
    const handleChange = (e) => {
        ...
        if (name === "phone") {
            ...
            let formattedValue = limitedValue.replace(/(\d{4})(\d{0,3})/, (_, p1, p2) => {
                return [p1, p2].filter(Boolean).join(' ');
            });
            // Format the phone number  
            if (limitedValue.length < 5) {
                formattedValue = limitedValue.replace(/(\d{0,4})/, (_, p1) => {
                    return [p1].filter(Boolean).join(' ');
                });
            } else if (limitedValue.length < 7) {
                formattedValue = limitedValue.replace(/(\d{4})(\d{0,2})/, (_, p1, p2) => {
                    return [p1, p2].filter(Boolean).join(' ');
                });
            } else if (limitedValue.length < 8) {
                formattedValue = limitedValue.replace(/(\d{4})(\d{0,3})/, (_, p1, p2) => {
                    return [p1, p2].filter(Boolean).join(' ');
                });
            } else if (limitedValue.length < 11) {
                formattedValue = limitedValue.replace(/(\d{4})(\d{3})(\d{0,3})/, (_, p1, p2, p3) => {
