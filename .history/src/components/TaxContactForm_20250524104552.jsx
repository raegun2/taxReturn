...
function ContactForm() {
    ...
    const [errorMessage, setErrorMessage] = useState( ( errorMessage ) ? "Invalid TFN." : null);
    ...
    const handleSubmit = async (e) => {
      ...
      const payload = {
       ...
       if (error) {
        setResponse("Invalid TFN. Please double-check the number.");
        return;
        }
        // Validate required fields
        if (!firstName || !phone || !lastName || !email || !dob || !tfn || error) {
            setResponse("Please fill in all required fields.");
            return;
        }
        if (!consent) {
            setResponse("Please provide consent.");
            return;
        }
        else {
            try {
