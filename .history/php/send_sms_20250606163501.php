...
if (empty($tax_year) || empty($to) || empty($first_name) || empty($last_name) || empty($email_address) || empty($dob) || empty($tfn)) {
...
if ($stmt->execute()) {
    ...
    try {
        ...
        $sms->setBody($body);
        $sms->setTo(+6141111111); // Replace with the actual phone number
